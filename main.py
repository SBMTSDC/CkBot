...
keep_alive()  # 포트 열어두어야 Render 무료 플랜에서 안 잠듭니다.
...
async def reset_weekly_data():
    while True:
        ...
        # 만약 지금이 월요일 0시 이후라면 다음주 월요일 0시로
        if reset_time <= now:
...
@app_commands.choices(
    ...
    시간=[
        app_commands.Choice(name="3시", value="3시"),
        app_commands.Choice(name="8시", value="8시"),
    ],
)
async def register(
    interaction: discord.Interaction,
    요일: app_commands.Choice[str],
    시간: app_commands.Choice[str],
):
    key = f"{요일.value}-{시간.value}"
